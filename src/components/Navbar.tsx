  ...
  return (
    <header
      className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${
        isScrolled
          ? 'bg-[#04070d]/95 backdrop-blur-xl border-b border-amber-500/20 py-2.5 sm:py-3 shadow-2xl shadow-amber-950/30'
          : 'bg-gradient-to-b from-[#04070d]/98 via-[#04070d]/70 to-transparent py-3 sm:py-4'
      }`}
    >
      <div className="max-w-7xl mx-auto px-3 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between gap-2 sm:gap-4 h-11">
          ...
          {/* Desktop Navigation Links Pill (Visible on XL screens) */}
          <nav className="hidden xl:flex items-center gap-1 bg-[#0f1523]/90 px-3 py-1.5 rounded-full border border-amber-500/20 shadow-lg">
            {navLinks.map((link) => (
              <a
                key={link.name}
                href={link.href}
          ...
          {/* Action Controls (Visible on Large screens md+) */}
          <div className="hidden md:flex items-center gap-2 sm:gap-2.5 h-full">
            ...
            <div className="flex items-center bg-[#0f1523]/90 rounded-xl p-1 border border-amber-500/25 h-9">
              ...
              {(['PKR', 'USD', 'AED'] as const).map((curr) => (
                <button
                  ...
                  className={`px-2 py-0.5 rounded-lg font-black transition-all text-[11px] whitespace-nowrap h-7 ${
                    activeCurrency === curr
            ...
            <a
              ...
              className="flex items-center gap-1.5 text-xs font-bold text-slate-200 hover:text-white px-2.5 sm:px-3 h-9 rounded-xl bg-[#0f1523]/90 border border-amber-500/25 hover:border-amber-400/50 transition whitespace-nowrap"
              title="Call Helpline: 0333 1588959"
            >
              <Phone className="w-3.5 h-3.5 text-[#E5983A] flex-shrink-0" />
              <span className="whitespace-nowrap text-xs">0333 1588959</span>
            </a>

            {/* WhatsApp Booking CTA */}
            <button
              onClick={onOpenBooking}
              className="flex items-center justify-center gap-1.5 sm:gap-2 bg-gradient-to-r from-[#E5983A] via-amber-500 to-[#D97706] hover:from-amber-400 hover:to-amber-500 text-slate-950 text-xs font-black px-3.5 sm:px-4 h-9 rounded-xl shadow-lg shadow-amber-950/60 transition-all hover:scale-105 active:scale-95 whitespace-nowrap flex-shrink-0"
            >
          ...
          <div className="flex items-center gap-2 xl:hidden">
            <button
              onClick={onOpenBooking}
              className="md:hidden flex items-center justify-center gap-1 bg-[#E5983A] text-slate-950 text-xs font-black px-2.5 py-1.5 rounded-lg shadow-md whitespace-nowrap"
            >
            ...
            <button
              ...
              className="p-2 rounded-xl bg-[#0f1523] text-slate-200 border border-amber-500/30 hover:text-white focus:outline-none"
        ...
        {mobileMenuOpen && (
          <div className="xl:hidden mt-3 p-4 sm:p-5 bg-[#04070d]/98 border border-amber-500/30 rounded-3xl backdrop-blur-2xl shadow-2xl space-y-4 animate-in slide-in-from-top duration-200">
            ...
            <div className="space-y-1 pt-2 border-t border-slate-800">
              {navLinks.map((link) => (
                <a
                  ...
                  className="flex items-center gap-3 px-3.5 py-2.5 text-sm font-bold text-slate-200 hover:text-[#E5983A] hover:bg-slate-900 rounded-xl transition"
                >
                  {link.name}
                </a>
              ))}
